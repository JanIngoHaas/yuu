use crate::{
    ...
    pass_type_inference::{infer_binding, pass_type_inference_impl::TransientData},
...
fn infer_enum_pattern(
    ...
    // First, try to resolve the enum pattern - this validates it exists AND was actually registered as a enum (not just a struct...)
    let enum_info = match data.type_registry.resolve_enum(enum_pattern.enum_name) {
        Some(info) => info,
        None => {
            let error = YuuError::builder()
                ...
                .span(
                    ...
                    format!("enum '{}' not defined", enum_pattern.enum_name),
                )
    ...
    // struct_type -> get variant info
    match scrutinee_ty {
        TypeInfo::Struct(struct_type) => {
            // Check if this struct type represents an enum and if this pattern matches
            if struct_type.name != enum_pattern.enum_name {
                let err_msg = YuuError::builder()
                    .kind(ErrorKind::TypeIncompatible)
                    .message(format!(
                        "Cannot match '{}' pattern against '{}' enum type",
                        enum_pattern.enum_name, struct_type.name
                    ))
                    .source(
                        data.src_code.source.clone(),
                        data.src_code.file_name.clone(),
        ...
        _ => {
            let err_msg = YuuError::builder()
                ...
                .span(
                    enum_pattern.span.clone(),
                    format!("has type '{}', not an enum", scrutinee_ty),
                )
