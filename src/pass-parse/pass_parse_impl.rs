...
impl Parse {
    ...
    pub fn run(&self, source_info: &SourceInfo) -> miette::Result<(AST, crate::pass_parse::add_ids::IdGenerator, SyntaxErrors)> {
        let mut parser = Parser::new(source_info);
        let (ast, _expr_count, id_generator) = parser.parse_and_add_ids();
