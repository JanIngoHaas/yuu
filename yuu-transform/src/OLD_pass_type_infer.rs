...
// use yuu_parse::parser::{SourceCodeInfo, SrcCache};
// use yuu_shared::ast::*;
// use yuu_shared::token::{Integer, Token, TokenKind};
// use yuu_shared::{Context, Pass};

// use crate::pass_type_inference::{BindingTable, Block};
// use crate::mangling::{mangle_func_name_with_binding, mangle_func_name_with_type};
// use crate::semantic_error::{GenericSemanticMsg, Note, NoteType, SemanticMsg, Severity};
// use crate::type_info::{BuiltInType, FunctionType, TypeInfo, TypeInfoTable};

// pub struct LocalTypeInferPass {
//     pub(crate) src_cache: SrcCache,
//     pub(crate) failed: bool,
//     pub(crate) type_info_table: TypeInfoTable,
// }

// impl LocalTypeInferPass {
...
//     pub fn infer_expr(&mut self, bin_expr: &ExprNode, bindings: &BindingTable) -> TypeInfo {
//         todo!()
//     }

//     pub fn infer_bin_expr(&mut self, bin_expr: &BinaryExpr, bindings: &BindingTable) -> TypeInfo {
//         let lhs = self.infer_expr(bin_expr.left.as_ref(), bindings);
//         let rhs = self.infer_expr(bin_expr.right.as_ref(), bindings);

//         // First, we look up the binding of the operator
//         let (bound_to, bound_to_span) = bindings
//             .bindings
//             .get(&bin_expr.id)
//             .expect("no binding found - ICE");

//         // Now, we look in the type info table to see if the expression follows the type of the binding
//         let expected_ty = self
//             .type_info_table
//             .types
//             .get(bound_to)
//             .expect("no type found - ICE");

//         let actual_ty = TypeInfo::Function(FunctionType {
//             args: vec![lhs, rhs],
//             ret: Box::new(expected_ty.clone()),
//         });

//         // // If the types don't coerce, we emit an error

//         // if !expected_ty.does_coerce_to_same_type(&actual_ty) {
//         //     let error = GenericSemanticMsg {
//         //         message: "Type mismatch in expression".to_string(),
//         //         cache: self.src_cache.clone(),
//         //         span: bin_expr.span.clone(),
//         //         notes: vec![
//         //             Note {
//         //                 message: format!("This expression has type `{}`", actual_ty),
//         //                 span: None,
//         //                 note_type: NoteType::Explanation,
//         //             },
//         //             Note {
//         //                 message: format!(
//         //                     "But it is bound to the variable `{}` with type `{}`",
//         //                     bound_to, expected_ty
//         //                 ),
//         //                 span: bound_to_span.clone(),
//         //                 note_type: NoteType::Explanation,
//         //             },
//         //         ],
//         //         severity: Severity::Error,
//         //     };
//         //     SemanticMsg::Generic(error).eprint();
//         //     return TypeInfo::BuiltIn(BuiltInType::Error);
//         // } else {
//         //     actual_ty
//         // }

//         // // Try to coerce the types
//         // if !lhs.does_coerce_to_same_type(&rhs) {
//         //     let error = GenericSemanticMsg {
//         //         message: "Type mismatch in binary expression".to_string(),
//         //         cache: self.src_cache.clone(),
//         //         span: bin_expr.span.clone(),
//         //         notes: vec![
//         //             Note {
//         //                 message: format!("This expression has type `{}`", lhs),
//         //                 span: Some(bin_expr.left.span()),
//         //                 note_type: NoteType::Explanation,
//         //             },
//         //             Note {
//         //                 message: format!("This expression has type `{}`", rhs),
//         //                 span: Some(bin_expr.right.span()),
//         //                 note_type: NoteType::Explanation,
//         //             },
//         //             Note {
...
//     pub fn infer_unary_expr(&mut self, unary_expr: &UnaryExpr) -> TypeInfo {
//         let operand = self.infer_expr(unary_expr.operand.as_ref());
//         operand
//     }
// }
