...
impl Scheduler {
    ...
    pub fn run(&self, schedule: Schedule) -> Result<(), String> {
        ...
        for (pass_a_name, _pass_a) in &schedule.passes {
            ...
            for (pass_b_name, _pass_b) in &schedule.passes {
                ...
                if let Some(outputs_pass_a) = schedule.resource_outputs.get(pass_a_name) {
